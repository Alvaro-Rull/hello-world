...
a_number <- "10"
strange_characters <- "@!-/"
all_together <- "Â¿CuÃ¡l es la capital de EspaÃ±a?"
...
# Puedo escribir 
# sin entero sin ir línea por línea haciendo ctrl + shift + C
# doesn' work
...
#Los corchetes se usan para acceder a elementos de un conjunto
...
#Logical vector <- myset[c(TRUE,FALSE,TRUE,FALSE)] exluye los FALSE
...
my_data_frame <- data.frame(
  ...
  #DATA FRAME = TABLA (Colecciones de columnas (vectores) con la misma longitud)
  
  col1 = 1:4, # = c(1.2.3.4)
  col2 = c("Category 1", "Category 2", "Category 3", "Category 4"), 
  col3 = c(3.4, -2.5, 10.1, -0.05), 
  col4 = c(T, F, F, T) #T es TRUE y F false
)
...
#Nombre del df(data frame) $ nombre de columna

class(my_data_frame$col3)

my_data_frame$col3+45

#3 formas de acceder a la misma columna
my_data_frame[[3]]
my_data_frame$col3
my_data_frame[["col3"]]
